...
#----------------------------------------------------------------------------
# Generator network used in the paper with mask.
...
def G_mask(
    ...
    is_template_graph   = True,        # True = template graph constructed by the Network class, False = actual evaluation.
    **kwargs):                          # Ignore unrecognized keyword args.
    ...
    # Building blocks.
    def mask(x):
        #  p = tf.get_variable(name=x.op.name+'_MaskParams',
                            #  shape=x.shape.as_list()[1:],
                            #  initializer=tf.constant_initializer(np.ones(shape=x.shape.as_list()[1:])))
        p = tf.placeholder_with_default(tf.constant(np.ones(shape=x.shape.as_list()[1:], dtype=np.float32)), 
                                        shape=x.shape.as_list()[1:], name='MaskParams')
        u = tf.random.uniform(p.shape,0,1)
        ...
        with tf.variable_scope('%dx%d' % (2**res, 2**res)):
            if res == 2: # 4x4
                ...
                with tf.variable_scope('Dense'):
                    ...
                    x = PN(mask(act(apply_bias(x))))
                with tf.variable_scope('Conv'):
                    x = PN(mask(act(apply_bias(conv2d(x, fmaps=nf(res-1), kernel=3, use_wscale=use_wscale)))))
            else: # 8x8 and up
                x = upscale2d(x)
...
#----------------------------------------------------------------------------
# Generator network used in the paper with minibatch mask.
