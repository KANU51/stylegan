...
#----------------------------------------------------------------------------
# Style-based generator with Bernoulli masks.
# Composed of two sub-networks (G_mapping and G_synthesis) that are defined below.
...
def G_style_masked(
    ...
    **kwargs):                                      # Arguments for sub-networks (G_mapping and G_synthesis).
    ...
    # Setup components.
    if 'synthesis' not in components:
        components.synthesis = tflib.Network('G_synthesis', func_name=G_synthesis_mask, **kwargs)
    num_layers = components.synthesis.input_shape[1]
    dlatent_size = components.synthesis.input_shape[2]
    if 'mapping' not in components:
        components.mapping = tflib.Network('G_mapping', func_name=G_mapping_mask, dlatent_broadcast=num_layers, **kwargs)
...
def mask(x, pixelwise=True, use_placeholder=True):
    if use_placeholder==True:
        if pixelwise:
            ...
            p = tf.get_variable(name='MaskParams', 
